...
from tracking import bbox_correction
...
class HIPTrack(BaseTracker):
    ...
    def initialize(self, image, info: dict, videoName: str):
        ...
        self.USE_CORRECTION = True  # HRatioCorrectionModule 켜기/끄기 플래그. ablation 위해 일단 끈 상태로 제출, 추후 켜볼 예정.
        self.USE_RECOVERY   = True  # SwitchRecoveryModule은 일단 보류, HRatioCorrectionModule의 효과를 순수하게 보기 위해서. 추후 ablation에서 켜볼 예정.

        import switch_recovery
        print(">>> using switch_recovery from:", switch_recovery.__file__)
        from switch_recovery import SwitchRecoveryModule
        self.recovery_module = SwitchRecoveryModule()

        import bbox_correction
        print(">>> using bbox_correction from:", bbox_correction.__file__)
        from bbox_correction import HRatioCorrectionModule
        self.correction_module = HRatioCorrectionModule(init_h=info['init_bbox'][3])
    ...
    def track(self, image, info: dict = None, dynamicUpdateTemplate=False, gt_crop=False):
        ...
        #pdb.set_trace()
        if self.frame_id == 0:
            print(">>> track() running from:", __file__)
...
#
        ...
        # [1] HRatioCorrectionModule
        corr_info = {"corrected": False}
        if self.USE_CORRECTION:
            self.state, corr_info = self.correction_module.update(response_max, list(self.state))
            if corr_info["corrected"]:
                self.state = clip_box(list(self.state), H, W, margin=10)

        topk_states = []

        # [2] SwitchRecoveryModule
        if self.USE_RECOVERY:
            bb = out_dict['backbone_feat']
            grid = self.params.search_size // 16
            feat_len_s = grid * grid
            search_feat = bb[:, -feat_len_s:, :]
            search_feat = search_feat.reshape(1, grid, grid, -1)
            c0 = grid // 3
            c1 = grid - grid // 3
            center = search_feat[:, c0:c1, c0:c1, :]
            feat_vec = center.reshape(-1, center.shape[-1]).mean(dim=0).cpu().detach().numpy()
            #if self.frame_id == 980:
            #    print(">>> CENTER POOLING ACTIVE, feat_vec shape:", feat_vec.shape, "file:", __file__)
            corrected, info = self.recovery_module.update(response_max, list(self.state), feat_vec=feat_vec)
            #if 940 <= self.frame_id <= 1389:
            #    print(f"f{self.frame_id} state={info['state']} rel={info.get('rel')} sim={getattr(self.recovery_module,'_last_sim',None)} rec={info.get('recovered')}")
            if info.get("recovered"):
                self.state = clip_box(list(corrected), H, W, margin=10)
        
        log_dir = "./confidence_log_0609_1320"
        os.makedirs(log_dir, exist_ok=True)
        log_path = os.path.join(log_dir, f"{self.seqName}_conf.csv")
        if self.frame_id == 1 and not os.path.exists(log_path):
            with open(log_path, "w") as f:
                f.write("frame,score_max,response_max,x,y,w,h,corrected\n")
        with open(log_path, "a") as f:
            x, y, w, h = self.state
            corrected_flag = 1 if corr_info.get("corrected") else 0
            f.write(f"{self.frame_id},{score_max:.6f},{response_max:.6f},"
                    f"{x:.2f},{y:.2f},{w:.2f},{h:.2f},{corrected_flag}\n")
