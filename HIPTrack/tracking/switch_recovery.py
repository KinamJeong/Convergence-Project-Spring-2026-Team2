...
class SwitchRecoveryModule:
    ...
    def __init__(self,
                 # --- 조건1: switch 감지 ---
                 ratio=0.75, persist=3, warmup=30, # ratio 0.65 -> 0.90 상향 (trained)
                 baseline_window=50, update_thresh=0.85,
                 # --- LOST 확정 / 복구 ---
                 lost_after=7,        # SUSPECT가 이만큼 지속되면 LOST(진짜 switch) 확정
                 motion_lag=6,       # motion 추정에 쓸 '오염 전' 시점 (SUSPECT 진입 기준 과거)
                 motion_span=10,      # 등속 속도 산출 구간 길이
                 recover_window=40,   # 복구 후 confidence 회복을 기다리는 최대 프레임
                 recover_ratio=0.95,  # 0.75 -> 0.95이 비율 이상으로 conf 회복되면 복구 성공
                 max_motion_speed=1.0,
                 feat_ratio=0.90, #90 
                 feat_warmup=30, 
                 feat_window=50): 
        # 감지 파라미터
